...
@app.route('/')  # 127.0.0.7:5000
def index():
    # streamer tension card
    tension_last_date = TensionAvg.query.with_entities(TensionAvg.date_).order_by(desc(TensionAvg.date_)).first()
    tension = TensionAvg.query.with_entities(TensionAvg.tension).filter(TensionAvg.date_ == tension_last_date).order_by(TensionAvg.streamer).all()

    # ins test card
    out_of_limit = [0, 0, 0, 0, 0, 0]
    upd_date = InsTest.query.with_entities(cast(InsTest.updated, Date)).order_by(desc(InsTest.updated)).first()
